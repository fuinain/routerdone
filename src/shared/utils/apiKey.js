...
const API_KEY_SECRET_FILE = path.join(DATA_DIR, "api-key-secret");

function loadApiKeySecret() {
  if (process.env.API_KEY_SECRET) return process.env.API_KEY_SECRET;
  try {
    const existing = fs.readFileSync(API_KEY_SECRET_FILE, "utf8").trim();
    if (existing) return existing;
  } catch {}
  fs.mkdirSync(DATA_DIR, { recursive: true });
  const generated = crypto.randomBytes(32).toString("hex");
  fs.writeFileSync(API_KEY_SECRET_FILE, generated, { mode: 0o600 });
  return generated;
}
