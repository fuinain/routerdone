  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "routerdone")
        : path.join(os.homedir(), ".routerdone"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all routerdone-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from routerdone install dir or wrapper cli.js
...
// Resolve npx/routerdone binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx routerdone` — works regardless of global bin path changes after npm i -g
