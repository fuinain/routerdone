...
// Check if settings has RouterDone customModels
const hasRouterDoneConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:RouterDone"));
};
...
// POST - Update RouterDone customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing RouterDone configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:RouterDone"));
...
// DELETE - Remove RouterDone customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove RouterDone customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:RouterDone"));
