...
const hasRouterDoneConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["routerdone"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("routerdone");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasRouterDone: hasRouterDoneConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "RouterDone", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "RouterDone settings removed from Kilo Code" });
  } catch (error) {
