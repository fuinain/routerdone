...
const hasRouterDoneConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "RouterDone");
};

const getRouterDoneEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "RouterDone") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getRouterDoneEntry(config);
...
// POST - Apply RouterDone config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing RouterDone entry or append
    const idx = config.findIndex((e) => e.name === "RouterDone");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove RouterDone entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "RouterDone");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "RouterDone removed from Copilot config",
    });
  } catch (error) {
