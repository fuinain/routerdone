...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-routerdone.env");
};
...
const hasRouterDoneConfig = (config) => {
  ...
  if (providers["routerdone"]) return true;
...
export async function GET() {
  ...
  const hasRouterDone = hasRouterDoneConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_ROUTERDONE_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile routerdone",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["routerdone"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_ROUTERDONE_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "routerdone configuration removed from jcode",
    });
  } catch (error) {
