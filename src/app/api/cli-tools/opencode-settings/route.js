...
const hasRouterDoneConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["routerdone"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["routerdone"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasRouterDone: hasRouterDoneConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("routerdone/") ? config.model.replace(/^routerdone\//, "") : null,
...
// POST - Apply RouterDone as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_routerdone";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing routerdone provider entry and its models
    const existingProvider = config.provider["routerdone"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["routerdone"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `routerdone/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `routerdone/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("routerdone/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove RouterDone provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["routerdone"]?.models) {
      delete config.provider["routerdone"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["routerdone"].models).length === 0) {
        delete config.provider["routerdone"];
        if (config.model?.startsWith("routerdone/")) delete config.model;
      } else if (config.model === `routerdone/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["routerdone"].models);
        config.model = `routerdone/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire routerdone provider
      if (config.provider) delete config.provider["routerdone"];
      if (config.model?.startsWith("routerdone/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("routerdone/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "RouterDone settings removed from OpenCode",
    });
  } catch (error) {
