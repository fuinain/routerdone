...
// Check if settings has RouterDone config
const hasRouterDoneConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["routerdone"];
};

// Read per-agent models.json and return current model id (without "routerdone/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["routerdone"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasRouterDone: hasRouterDoneConfig(settings),
...
// POST - Update RouterDone settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `routerdone/${model}`;

    // Remove all old routerdone/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("routerdone/"))
    ...
    // Add fresh routerdone models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`routerdone/${m}`] = {};
    });

    // Remove old routerdone model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("routerdone/")) {
    ...
    // Update models.providers.routerdone with all models
    settings.models.providers["routerdone"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `routerdone/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove RouterDone settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove RouterDone from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["routerdone"];
    ...
    // Remove routerdone models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("routerdone/"));
    ...
    // Reset agents.defaults.model.primary if it uses routerdone
    if (settings.agents?.defaults?.model?.primary?.startsWith("routerdone/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "RouterDone settings removed successfully",
    });
  } catch (error) {
