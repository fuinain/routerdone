...
export default function CombosPage() {
  ...
  const handleSetComboStrategy = async (comboName, patch) => {
    try {
      ...
      const fallbackStrategy = next.fallbackStrategy || "fallback";
      const retryAttempts = Number(next.retryAttempts ?? DEFAULT_COMBO_RETRY_ATTEMPTS);
      const retryDelayMs = Number(next.retryDelayMs ?? DEFAULT_COMBO_RETRY_DELAY_MS);
      const usesDefaultRetry = retryAttempts === DEFAULT_COMBO_RETRY_ATTEMPTS && retryDelayMs === DEFAULT_COMBO_RETRY_DELAY_MS;

      // Prune to keep settings clean: default fallback with default retry = no entry.
      if (fallbackStrategy === "fallback" && usesDefaultRetry && !next.judgeModel && !next.fusionTuning) {
        delete updated[comboName];
      } else {
        updated[comboName] = { ...next, fallbackStrategy, retryAttempts, retryDelayMs };
      }
...
function ComboCard({ combo, modelCaps = {}, activeProviders = [], copied, onCopy, onEdit, onDelete, strategy = {}, onSetStrategy }) {
  ...
  const retryAttempts = Number.isFinite(Number(strategy.retryAttempts)) ? Number(strategy.retryAttempts) : DEFAULT_COMBO_RETRY_ATTEMPTS;
  const retryDelayMs = Number.isFinite(Number(strategy.retryDelayMs)) ? Number(strategy.retryDelayMs) : DEFAULT_COMBO_RETRY_DELAY_MS;
    ...
    <Card padding="sm" className="group">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3 sm:shrink-0">
          ...
          {!isFusion && (
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-black/[0.02] px-2 py-1.5 dark:bg-white/[0.02] sm:w-[180px]">
              <label className="flex min-w-0 flex-col gap-0.5">
                <span className="text-[10px] font-medium text-text-muted">Retry/model</span>
                <input
                  type="number"
                  min="0"
                  max="10"
                  value={retryAttempts}
                  onChange={(e) => onSetStrategy({ retryAttempts: Math.max(0, Math.min(Number(e.target.value || 0), 10)) })}
                  className="h-7 w-full rounded border border-black/10 bg-white px-2 text-xs text-text-main outline-none focus:border-primary dark:border-white/10 dark:bg-black/20"
                  title="Retries on the same combo model before trying the next model. 0 means immediate fallback."
                />
              </label>
              <label className="flex min-w-0 flex-col gap-0.5">
                <span className="text-[10px] font-medium text-text-muted">Delay (s)</span>
                <input
                  type="number"
                  min="0"
                  max="30"
                  ...
                  onChange={(e) => onSetStrategy({ retryDelayMs: Math.round(Math.max(0, Math.min(Number(e.target.value || 0), 30)) * 1000) })}
                  disabled={retryAttempts === 0}
                  className="h-7 w-full rounded border border-black/10 bg-white px-2 text-xs text-text-main outline-none focus:border-primary disabled:cursor-not-allowed disabled:opacity-45 dark:border-white/10 dark:bg-black/20"
                  title="Delay between retries on the same model. Ignored when Retry/model is 0."
                />
              </label>
            </div>
          )}
