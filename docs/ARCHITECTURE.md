# RouterDone Architecture

_Last updated: 2026-02-06_

## Executive Summary

RouterDone is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[RouterDone Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.routerdone/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.routerdone/usage.json`, `~/.routerdone/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.routerdone/db.json`)
- usage stats: `~/.routerdone/usage.json`
- request log lines: `~/.routerdone/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[RouterDone Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.routerdone` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/routerdone && npm run build`
- Build Docker image: `cd /root/dev/routerdone && docker build -t routerdone .`
