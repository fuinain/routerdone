...
import { HTTP_STATUS, shouldForceStreamUpstream } from "../config/runtimeConfig.js";
import { handleBypassRequest } from "../utils/bypassHandler.js";
import { resolveRoutePolicy } from "../services/routePolicy.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, sourceFormatOverride, providerThinking, routeInfo = null, streamTimeoutPolicy = null, streamPreflightTimeoutMs = null }) {
  ...
  PROVIDERS[provider]?.forceStream === true
  || shouldForceStreamUpstream(provider, model);
  ...
  if (headersDeadlineMs) {
    headersTimer = setTimeout(() => {
      upstreamHeadersTimedOut = true;
      log?.warn?.("STREAM", `${provider?.toUpperCase?.() || provider} | ${model} | upstream headers timeout (${headersDeadlineMs}ms)`);
  ...
  try {
    ...
    saveRequestDetail(buildRequestDetail({
      provider, model, connectionId, routeInfo,
    ...
    if (error.name === "AbortError") {
      ...
      if (upstreamHeadersTimedOut) return createErrorResult(HTTP_STATUS.BAD_GATEWAY, "Upstream headers timeout");
